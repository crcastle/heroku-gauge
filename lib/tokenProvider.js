...
var TokenProvider = (function(){
  ...
  TokenProvider.prototype.getActiveToken = async function() {
    ...
    if(this.currentToken && new Date(this.currentToken.expires_in_date) > new Date()){
      debug(`Token in memory and not expired: %o`, this.currenToken);
    ...
    debug(`Refresh token:`, this.currentToken.refresh_token);
    debug(`Client secret:`, config.oauth.heroku.secret);
    const response = await fetch(config.oauth.heroku.tokenUrl, {
      method: 'POST',
      body: JSON.stringify({
        grant_type: 'refresh_token',
        refresh_token: this.currentToken.refresh_token,
        client_secret: config.oauth.heroku.secret
      }),
      headers: {
